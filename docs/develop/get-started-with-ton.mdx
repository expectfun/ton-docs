...
Though the miner will work properly after the completion of the above steps, it will appear very poor visually. Therefore, we must address this issue. Let’s jump in.
...
To achieve this goal, we’ll initiate the creation of a payment url (a payment link) using the `ton://transfer/<address>` format that can be customized using different parameters. This will work perfectly because we want to create a message and send it to the smart contract within our Tonkeeper wallet:
...
* [[YouTube] Ton Dev Study - FunC & Blueprint](https://www.youtube.com/playlist?list=PLtUBO1QNEKwtO_zSyLj-axPzc9O9rkmYa)
